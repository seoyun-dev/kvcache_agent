...
- developer_adoption: TurboQuant/InfiniGen 각각에 대해 도입 개발자가 뭐라고
  말했는지 (채택했다고 말함 / 조건부 / 안 쓴다고 말함 / 근거 없음). 시장성과
  같은 세 런타임(llama.cpp·MLX-VLM·vLLM) 중 온디바이스 도메인이므로
  **llama.cpp를 대표로 본다.** 구현자가 지목한 채택 장벽이 있으면 notes에 적는다.
- investor_coverage: TurboQuant/InfiniGen 각각 애널리스트·미디어가 다뤘는지
  (있음 / 근거 없음). 보도자료 전재·벤더 발표는 세지 않는다.
- 세 항목 모두 두 기술 값을 각각 채운다. notes에는 **TurboQuant·InfiniGen
  각 기술당 지지 1건 + 비판 1건, 총 4건**을 주체·원문 요지와 함께 적는다
  (항목별로 6건씩이 아니라 기술 단위로 4건 - 어느 항목에서 나온 근거인지는
  괄호로 표시). 한쪽 근거가 없으면 "반대 근거 없음(찾아본 범위: OO)"이라고
  명시한다.
- label 도출 규칙: 세 항목을 하나씩 놓고, 그 항목의 실제 근거 내용이 어느
  기술에 더 유리한 신호인지 먼저 개별 판단한다(카테고리 순서가 아니라
  내용 기준 - 예: 한계를 지적받은 쪽이 그 항목에서 불리). 세 항목 방향이
  전부 같으면 그 방향을 label로, 갈리면 "조건 의존"(어느 항목이 어느
  쪽인지 notes에 명시), 판단 가능한 항목이 없으면 "판단보류".
