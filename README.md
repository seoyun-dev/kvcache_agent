...
* **환경이 다른 측정치를 섞지 않는다**
  TRL은 trl\_ondevice/trl\_global을 분리해, 데이터센터급 GPU에서 잰 결과를 온디바이스 근거로 착각하지 않게 환경 게이트(TRL 5부터)를 걸었다. 도메인 평가도 InfiniGen이 전제하는 host DRAM이 기준 기기(RAM 8GB 스마트폰)에 없다는 사실을 메모리 예산 판정에 직접 반영한다.

* **검증 실패를 조용히 넘기지 않는다**
  H가 재시도 상한(2회)에 도달해도 무조건 통과시키되, 무엇을 못 채웠는지를 보고서 안(REFERENCE 장 앞)에 감사 노트로 자동 남긴다. 콘솔 로그로만 남으면 최종 산출물만 보는 사람은 검증 실패 사실 자체를 알 수 없다.
...
* **LLM-as-Judge 재검증 루프(B'/E')를 시간 관계상 못 넣었다 — 이게 남긴 진짜 한계**
...
* **하드코딩된 검색 쿼리라 사실은 "Agentic RAG"가 아니다**
  B·E의 검색 쿼리는 코드에 고정돼 있고, 검색 결과가 부실해도 LLM이 스스로 "다시 검색할지/다른 쿼리를 쓸지" 판단하는 루프가 없다. 검색 결과 품질이 곧바로 판정 품질에 반영돼버리는 구조라, 실제로는 하이브리드 검색(BM25+FAISS+RRF)을 쓰는 **Advanced RAG**에 가깝다는 걸 뒤늦게 명확히 했다 — 용어를 정확히 쓰는 것 자체가 다음에 뭘 보강해야 하는지(검색 자체의 자기 교정)를 알려준다는 걸 배웠다.

* **"편향 방지"는 한 곳에 심어서 될 문제가 아니다**
  처음엔 프롬프트에 지시문만 추가하면 될 거라 생각했지만, 실제로는 스키마 설계(TechStatus, label 어휘 제한) + 프롬프트 + 규칙 기반 검증까지 세 군데를 동시에 손봐야 실효성이 생겼다. 방지책이 하나라도 빠지면 다른 층이 놓친 걸 못 잡는다는 걸 확인했다.

* **구조화 출력의 현실적 제약이 설계를 바꿨다**
  Synthesis.labels를 자유 형식 dict로 설계했더니 OpenAI strict structured output이 "필수 키를 사전에 알 수 없다"며 거부했다. 자유 형식을 고집하지 않고 {market, stakeholder, domain, trl} 4개 고정 키로 스키마를 확정해야 했다 — "유연한 스키마"가 항상 좋은 설계는 아니고, LLM 구조화 출력의 안정성이 스키마 설계를 실질적으로 제약한다는 걸 체감했다.

* **무엇을 자를지 정하는 것도 설계의 일부다**
  리랭커, B'/E' 재검증, 작동점 A/B를 각각 구조화 필드로 두 번 채우는 것 등은 전부 의도적으로 MVP 스코프에서 뺐다. "시간이 없어서 못 했다"가 아니라 "무엇이 핵심 신뢰도에 영향을 주고 무엇이 안 주는지"를 먼저 가리는 판단이 선행돼야, 자른 부분을 한계점으로 정직하게 적을 수 있다는 걸 배웠다.
